...
KNOWN_IPS_FILE="$HOME/alphaverse-xrp/bin/known_ips.txt"

cd $HOME/alphaverse-xrp/
echo $SUDO_PASSWORD | sudo -S echo hello
sshpass -p "$SUDO_PASSWORD" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${user}@${API_SERVER}:/home/${USER}/alphaverse-xrp/known_ips.txt $KNOWN_IPS_FILE
...
while true; do
	...
	sleep 2
	screen -dmS client && screen -S client -X stuff 'cd /mnt/alphaverse-xrp && npm run client\n'
	sleep 3
