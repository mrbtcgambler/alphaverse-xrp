...
CHANGE_SCRIPT="$HOME/alphaverse-xrp/bin/change.sh"

cd $HOME/alphaverse-xrp/
echo $SUDO_PASSWORD | sudo -S echo hello
sshpass -p "$SUDO_PASSWORD" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${user}@${API_SERVER}:/home/${USER}/alphaverse-xrp/bin/change.sh $CHANGE_SCRIPT
