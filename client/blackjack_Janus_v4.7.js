...
const persistentProfitPath = new URL('../janus_profit.json', import.meta.url); // Path for persisting profit
 ...
 */
const state = {
  version: 4.7, // Profit calculation is now derived from game outcomes, not balance changes.
  baseBet: config.recoverThreshold / 1000,
  startingBalance: config.recoverThreshold,
  ...
  IncreaseOnLoss: 2,
  currentBet: 0,
  profit: 0, // This will be loaded from the persistent file
  bets: 0,
  wager: 0,
  winCount: 0,
  currentStreak: 0,
...
// Load persisted profit on startup
try {
  if (existsSync(persistentProfitPath)) {
    const persisted = JSON.parse(await readFile(persistentProfitPath));
    if (typeof persisted.profit === 'number') {
        state.profit = persisted.profit;
        console.log(`Loaded persisted profit: ${state.profit.toFixed(8)}`);
    }
  }
} catch (e) {
  console.warn('[WARN] Could not load persisted profit, starting from 0.');
}
...
async function writeStatsFile(currentBalance) {
  ...
  try {
    ...
    // Persist the profit separately
    await writeFile(persistentProfitPath, JSON.stringify({ profit: state.profit }));
  } catch (e) {
    console.error('Failed to write stats/profit file:', e);
  }
}
...
async function handleStage1Logic(isWin, isLoss, currentBalance) {
    ...
    if (balance > (config.recoverThreshold + state.profit)) {
            const amountToVault = balance - (config.recoverThreshold + state.profit);
            console.log(`Initial balance is high. Vaulting ${amountToVault.toFixed(8)} ${config.currency}...`);
            apiClient.depositToVault(config.currency, amountToVault);
        }
...
(async () => {
  ...
  while (true) {
    try {
        ...
        // Keep balance fresh for bet sizing/ops, but do not use it for profit.
        const fundsAfterRound = await apiClient.getFunds(config.currency);
        balance = fundsAfterRound.available;

        const roundProfit = roundPnL; // For logs and win/loss determination
        state.profit += roundPnL;      // Accumulate profit from play only
